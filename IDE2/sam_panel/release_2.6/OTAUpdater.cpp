...
const char* versionURL = "https://api.djdevs.eu/firmware/ladislav/version.json";
...
void OTAUpdater::checkForUpdates(const String& currentVersion) {
  if (!upToDate(currentVersion)) {
    ...
    sendAsString("l001");
    sendAsString("zI " + currentVersion + "> new ver.");  //verze
...
void OTAUpdater::performOTAUpdate(const char* downloadURL) {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (Update.begin(contentLength)) {
      ...
      sendAsString("zI Aktualizace...");  //update
      delay(500);
      int written = 0;
      int prevProgress = 0;
      uint8_t buffer[1024];
          ...
          if (bytesRead > 0) {
            ...
            if (bytesWritten > 0) {
              ...
              if (progress != prevProgress) {
                ...
                sendAsString("zN" + String(progress) + "%");  //update
